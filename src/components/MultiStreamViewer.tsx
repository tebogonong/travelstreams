    ...
    <div className="relative w-full h-screen bg-black">
      ...
      {activeStreams.length > 1 && (
        <div className="absolute top-2 left-1/2 -translate-x-1/2 sm:top-4 z-40">
          <div className="bg-gradient-to-r from-yellow-500 to-orange-500 px-3 py-1 sm:px-6 sm:py-2 rounded-full shadow-lg backdrop-blur-sm">
      ...
      {activeStreams.length > 1 && (
        <div className="absolute top-12 left-1/2 -translate-x-1/2 sm:top-20 z-40">
          <Dialog>
            <DialogTrigger asChild>
              <Button 
                className="bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 shadow-lg text-xs sm:text-sm px-3 py-2 sm:px-4"
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>Bet on Winning Stream</DialogTitle>
                <DialogDescription>
                  ...
                  return (
                    <div key={stream.id} className="p-4 border rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <Badge style={{ backgroundColor: stream.tag.color }}>
                      ...
                      >
                        Bet $10 on {stream.tag.displayName}
                      </BasePay>
                    </div>
                  );
                })}
      ...
      {/* Stream Grid */}
      <div className={`grid ${getStreamContainerClass()} h-full gap-1`}>
        {activeStreams.map((stream, index) => {
          ...
          return (
            <div 
              key={stream.id} 
              className={`relative ${isLeading ? 'ring-4 ring-yellow-400' : ''}`}
            >
              <StreamPlayer
                stream={stream}
              ...
              {activeStreams.length > 1 && (
                <div className="absolute bottom-24 left-4 right-4 z-30 bg-black/80 backdrop-blur-sm rounded-lg p-3">
      ...
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 sm:bottom-4 z-40 max-w-[95vw] overflow-x-auto">
        <div className="flex gap-1 sm:gap-2 bg-black/80 backdrop-blur-sm p-1.5 sm:p-2 rounded-full">
